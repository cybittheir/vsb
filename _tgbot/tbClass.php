...
class telegramBot
{
  ...
  public function sendMessage($chat_id, $text, $disable_web_page_preview = false, $reply_to_message_id = null, $reply_markup = null)
  {
    ...
    return $this->sendMRequest('sendMessage', $params);
  }
 ...
 public static function forceReply($selective = false)
 {
  ...
  private function sendMRequest($method, $params)
  {
    # return json_decode(file_get_contents($this->baseURL . $method . '?' . http_build_query($params)), true);
    $result=json_encode($params, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_NUMERIC_CHECK | JSON_PRETTY_PRINT);
    // echo $result;
    print_r ($result);
    return $result;
  }
